...
- `subfield_channel_connectivity` reports how many retained subfields have at least one per-pixel flowpath that drains directly to a channel.
...
./target/release/subfield_channel_connectivity \
  --sub-field-map /path/to/run/ag_fields/sub_fields/sub_field_id_map.tif \
  --subwta /path/to/run/dem/wbt/subwta.tif \
  --wbt-flovec /path/to/run/dem/wbt/flovec.tif
```
...
`subfield_channel_connectivity` is read-only and takes every input resource explicitly; it does not infer a WEPPcloud run root. Its default channel test uses the established TOPAZ-style `SUBWTA` convention in which channel IDs end in `4`. Pass `--channel-mask <path>` to use positive cells in an explicit channel mask instead. The tool remaps the raw WhiteboxTools `FLOVEC` raster internally and emits deterministic JSON to standard output, or to `--out-json <path>`. Pass `--out-subfields-json <path>` to atomically write a separately versioned, sorted row for every retained subfield with its channel-connected flag and direct channel outlet-cell count.

The connectivity metric is intentionally direct: sub-field abstraction starts one flowpath at every retained subfield cell and stops each path after appending its first cell outside the subfield. A retained subfield qualifies when that first outside cell is a channel for at least one generated path. This establishes direct channel entry without claiming that intervening hillslope or buffer routing was modeled. The tool rejects inputs whose dimensions, affine geotransforms, or projections do not align.

...
`subfield_channel_connectivity` is read-only and takes every input resource explicitly; it does not infer a WEPPcloud run root. Its default channel test uses the established TOPAZ-style `SUBWTA` convention in which channel IDs end in `4`. Pass `--channel-mask <path>` to use positive cells in an explicit channel mask instead. The tool remaps the raw WhiteboxTools `FLOVEC` raster internally and emits deterministic JSON to standard output, or to `--out-json <path>`.

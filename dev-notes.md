...
- Supporting modules now sit in dedicated directories:
  ...
  - `src/rasters/d8_wbt_to_topaz.rs` reconciles D8 encoding differences between Whitebox Tools and TOPAZ.
  - `src/support/` hosts shared geometry helpers (`support.rs`) and the Douglas–Peucker simplifier (`douglas_peucker.rs`).

...
fn parse_field<T>(s: &str, line_num: usize, name: &str) -> io::Result<T>
where
    T: FromStr,
...
pub fn read_wbt_netw_tab<P: AsRef<Path>>(
    file_path: P,
    ...
    for (line_num, line) in reader.lines().enumerate().skip(1) {
        ...
        if links.insert(topaz_id, link).is_some() {
            eprintln!(
                "Warning: Duplicate topaz_id {} at line {}",
                topaz_id,
                line_num + 1
            );
        }

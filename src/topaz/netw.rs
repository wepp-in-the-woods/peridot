...
fn parse_line(line: &str, subwta: &Raster<i32>) -> Result<ChannelNode, Box<dyn std::error::Error>> {
    ...
    if chn_id0 == 0 {
        return Err(Box::new(std::io::Error::new(
            ...
            format!("chn_id0 is zero: {}", chn_id0),
        )));
    }
    if chn_id1 == 0 {
        return Err(Box::new(std::io::Error::new(
            std::io::ErrorKind::InvalidData,
            format!("chn_id1 is zero: {}", chn_id1),
        )));
    }
    if chnout_id == 0 {
        return Err(Box::new(std::io::Error::new(
            std::io::ErrorKind::InvalidData,
            format!("chnout_id is zero: {}", chnout_id),
        )));
    }
    if chn_id0 != chn_id1 {
        return Err(Box::new(std::io::Error::new(
            std::io::ErrorKind::InvalidData,
            format!("chn_id0 != chn_id1: {} {}", chn_id0, chn_id1),
        )));
    }
    if chn_id0 % 10 != 4 {
        return Err(Box::new(std::io::Error::new(
            std::io::ErrorKind::InvalidData,
            format!("chn_id0 not ending with 4: {}", chn_id0),
        )));
    }
    if chnout_id % 10 != 4 {
        return Err(Box::new(std::io::Error::new(
            std::io::ErrorKind::InvalidData,
            format!("chnout_id not ending with 4: {}", chnout_id),
        )));
    }
    if chn_id0 < chnout_id {
        return Err(Box::new(std::io::Error::new(
            std::io::ErrorKind::InvalidData,
            format!("chn_id0 < chnout_id: {} {}", chn_id0, chnout_id),
        )));
    }
...
pub fn write_network(
    file_path: &str,
    network: &HashMap<i32, HashSet<i32>>,
...
pub fn read_netw_tab(
    file_path: &str,
    subwta: &Raster<i32>,
) -> Result<(HashMap<i32, ChannelNode>, HashMap<i32, HashSet<i32>>), Box<dyn std::error::Error>> {
...
mod tests {
    ...
    fn test_read_netw_tab() {
        ...
        let file_path =
            "tests/fixtures/watershed_abstraction/mdobre-scarce-belch/dem/topaz/NETW.TAB";
